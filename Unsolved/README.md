...
### 内联部分Lambda表达式
如果一个需要内联的函数，多个参数类型都是函数类型，使用inline后，会将所有使用Lambda表达式的参数值都内联紧当前函数，要想使某些函数不内联进去需要使用`noinline`关键字。
### 非局部返回（still remain unsolved）
在Kotlin中，使用无限定符的return语句，只能用来退出一个有名称的函数或匿名函数。这就意味着，要退出一个Lambda表达式，必须使用一个标签。
